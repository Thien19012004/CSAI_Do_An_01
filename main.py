...
def bfs(grid, start, end):
    ...
    if not (0 <= start_y < len(grid) and 0 <= start_x < len(grid[0])) or not grid[start_y][start_x]:
        return None
    if not (0 <= end_y < len(grid) and 0 <= end_x < len(grid[0])) or not grid[end_y][end_x]:
        return None
...
class Pacman(MovingEntity):
    ...
    def input(self):
        ...
        if not self.onTheGrid() or not self.onTheGamePlay():
            return
        new_x_speed = 0
        new_y_speed = 0
    ...
    def update(self):
        if self.count < (self.time * 60):
            ...
            if self.state == self.chaseMode or self.state == self.scatterMode:
                self.modeTimer += 1
                if self.isChasing and self.modeTimer >= (60 * 20) or (not self.isChasing and self.modeTimer >= (60 * 10)):
...
class ClydeGhost(Ghost):
    def __init__(self, xPos, yPos):
        super().__init__(xPos, yPos, "inky.png", 4)
        self.chaseMode = ClydeChaseMode(self)  # Use BFS-based chase mode
...
class InkyGhost(Ghost):
    def __init__(self, xPos, yPos, otherGhost: BlinkyGhost = None):
        super().__init__(xPos, yPos, "clyde.png", 3)
...
class GhostState:
    ...
    def computeNextDir(self):
        new_x_speed = 0
        new_y_speed = 0
        if not self.ghost.onTheGrid() or not self.ghost.onTheGamePlay():
            return
        min_dist = float('inf')
...
class ClydeChaseMode(GhostState):
    def __init__(self, ghost: Ghost):
        super().__init__(ghost)
    ...
    def computeNextDir(self):
        if not self.ghost.onTheGrid() or not self.ghost.onTheGamePlay():
            return
        clyde_x = int(self.ghost.xPos // 8)
        clyde_y = int(self.ghost.yPos // 8)
        pacman_x = int(pacman.xPos // 8)
        pacman_y = int(pacman.yPos // 8)
        path = bfs(walkable_grid, (clyde_y, clyde_x), (pacman_y, pacman_x))
        if not path or len(path) < 2:
            super().computeNextDir()
            return
        ...
        if dx == 1:
            ...
            new_y_speed = -self.ghost.speed
        if not checkWallCollision(self.ghost, new_x_speed, new_y_speed):
            self.ghost.xSpeed = new_x_speed
            self.ghost.ySpeed = new_y_speed
        else:
            super().computeNextDir()
...
def checkGhostCollisionFrighten():
    global score, gameOver, pacman, ghosts
...
def init():
    global level, pacman, blinkGhost, inkyGhost, pinkyGhost, clydeGhost, ghosts, wall, totalFood, walkable_grid
...
def main():
    ...
    global musicPlaying, game_first_input, running
    init()
    timerCount = 0
    while running:
